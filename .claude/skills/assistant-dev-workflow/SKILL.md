...
наступали не один раз. Полная спецификация — `SPEC.md` (что и как сейчас)
и `PLAN.md` (roadmap, история решений по фазам), конвенции — `CLAUDE.md`;
этот файл дополняет их операционными деталями, которые проще один раз
закрепить, чем каждый раз выводить заново.
