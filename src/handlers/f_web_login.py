"""Команда /webcode (Phase 45, третья переделка входа в веб-версию) —
...
login_codes теперь ходит в Redis (не в память процесса) — код рождается
здесь, в процессе bot, а проверяется в процессе api (src/adapters/
web_auth.py), это разные контейнеры без общей памяти."""
