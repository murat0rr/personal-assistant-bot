...
async def create_rule(title: str, schedule_kind: str, schedule_value: dict) -> dict:
    async with async_session() as session:
        rule = RecurringTaskRule(
            title=title, schedule_kind=schedule_kind, schedule_value=schedule_value
        )
        session.add(rule)
...
async def materialize_due_rules(today: date) -> list[str]:
    """Раз в день (см. scheduler/jobs.py) — для каждого правила, чей
    паттерн подходит под сегодня и ещё не материализовано на сегодня,
    создаёт обычную Task-строку на сегодняшнюю дату. Дальше это уже
    просто задача — ни утренняя сводка, ни Mini App не нуждаются в
    отдельной логике под повторяющиеся, ровно как и просилось."""
    created_titles: list[str] = []
    async with async_session() as session:
        result = await session.execute(
            select(RecurringTaskRule).where(RecurringTaskRule.archived.is_(False))
        )
